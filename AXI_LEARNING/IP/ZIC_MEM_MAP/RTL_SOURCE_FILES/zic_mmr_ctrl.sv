...
(
     ...
     input logic [47:0] 	zic_int_pending_bit	    ,
     input logic            zic_int_pending_valid	,
     input logic [47:0]	    global_int_enable_bit	,
     input logic            global_int_enable_valid ,
    ...
    */
    ...
    (
     ...
     input logic 		zic_mmr_write_en		,
     input logic [31:0] 	zic_mmr_write_data		,
     input logic 		zic_int_pending_bit		,
     input logic 		zic_int_pending_bit_valid 	,
     input logic 		global_int_enable_bit		,
     input logic 		global_int_enable_valid 	,
     output logic	[31:0]  zic_irq_ctrl_o			
    
    );
    
      logic [7:0] zic_ie_mmr;
      logic [7:0] zic_ip_mmr;
      logic [7:0] zic_attr_mmr;
      logic [7:0] zic_ctl_mmr;
    
    
    always_ff@(posedge zic_clk or negedge zic_rst) //ie
    begin
    	if(!zic_rst)
    	begin
    		zic_ie_mmr <= 8'd0;
    ...
    always_ff@(posedge zic_clk or negedge zic_rst) //ip
    begin
    		if(!zic_rst)
    	begin
    		zic_ip_mmr <= 8'd0;
    ...
    always_ff@(posedge zic_clk or negedge zic_rst) //attr
    begin
    	if(!zic_rst)
    	begin
    		zic_attr_mmr <= 8'd0;
    ...
    always_ff@(posedge zic_clk or negedge zic_rst) //ctl
    begin
    	if(!zic_rst)
    	begin
