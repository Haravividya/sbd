class wdt_modes_test extends uvm_test;

  `uvm_component_utils(wdt_modes_test)

  wdt_env wdt_en;
  wdt_modes_seq wdt_seq;
//  logic [15:0] test_timer;
  ...
  task run_phase(uvm_phase phase);
    ...
    begin
      ...
      wdt_seq = wdt_modes_seq::type_id::create("wdt_seq",this);
     // test_timer    = 16'h0010;
    //  wdt_seq.timer = test_timer;
     if($value$plusargs("TEST_CASE=%d",test_case)==56)
      begin
      wdt_seq.case_sel = test_case;
      wdt_seq.start(wdt_en.wdt_agn.wdt_seqcr);
      repeat(30)@(posedge wdt_en.wdt_agn.wdt_drv.wdt_vif.clk);
      end
