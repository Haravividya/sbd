...
sub simulate_test 
{
  ...
  if(defined $cov_en) ######## Inside the xrun command change the compile_file path to your file and DUT_NAME with your dut instance name in your TB top file ######## 
    {
      system("xrun -access +rwc -f ../UVME/I2C_SLAVE/i2cs_compile.f -svseed $seed +UVM_TESTNAME=$test  +define+UVM_REPORT_DISABLE_FILE_LINE -coverage all -covdut i2c_slave_controller -covworkdir /cov_work -covoverwrite -covfile ./cov_files/cov_cmd.cf -uvmhome CDNS-1.1d"); 
    }
    else
    {  
      system("xrun -access +rwc -f ../UVME/I2C_SLAVE/i2cs_compile.f -svseed $seed +UVM_TESTNAME=$test  +define+UVM_REPORT_DISABLE_FILE_LINE -uvmhome CDNS-1.1d"); 
    }
