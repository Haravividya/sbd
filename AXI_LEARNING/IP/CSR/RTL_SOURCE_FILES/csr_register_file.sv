...
(
      ...
      input logic 		                    csr_set_bit		            ,
      input logic                           mret_valid_i                ,
      input logic 		                    csr_clear_bit		        ,
      input logic 		                    interrupt_valid 	        ,
      input logic  [DATA_WIDTH-1:0]	        interrupt_code		        ,
      input logic 		                    exception_valid 	        ,
      input logic  [DATA_WIDTH-1:0]	        exception_code		        ,
      input logic 		                    ld_sd_misalign_valid        ,
      input logic [DATA_WIDTH-1:0] 	        ld_sd_misalign_addr	        ,
      input logic 		                    instr_misalign_valid        ,
      input logic 			                trap_valid_i		        ,
      input logic 			                stall_valid_i		        ,
      input logic 			                branch_valid_i		        ,
      input logic [DATA_WIDTH-1:0] 	        instr_misalign_addr	        ,
      input logic 		                    mie_set			            ,//interrupt occurs
      input logic 		                    mie_clear		            ,//end of interrupt
      input logic 	[PC_WIDTH-1:0]	        csr_mepc_in			        ,
      input logic 		                    csr_mepc_write_valid        ,
      output logic  [PC_WIDTH-1:0]	        csr_mepc_o			        ,
      input logic 		                    invalid_instr_valid	        ,
      input logic  [INSTRUCTION_WIDTH-1:0]  invalid_instruction	        ,
      input logic [7:0]   	                interrupt_lvl_pr_i	        ,//from zic
      output logic [DATA_WIDTH-1:0] 	    interrupt_active_level_priority_o,
      input logic 		                    exception_id_write_valid_i  ,
      input logic		                    interrupt_id_write_valid_i  ,
      input logic  [PC_WIDTH-1:0]           pc			,
      output logic [DATA_WIDTH-1:0] 	    mvendorid_o		,	
      output logic [DATA_WIDTH-1:0] 	    marchid_o  		,     
      output logic [DATA_WIDTH-1:0] 	    mimpid_o   		,     
      output logic [DATA_WIDTH-1:0] 	    mhartid_o  		,     
      output logic [DATA_WIDTH-1:0] 	    mstatus_o  		,     
      output logic		                    mstatus_mie_o		,
      output logic [DATA_WIDTH-1:0] 	    misa_o	   		,     
      output logic [DATA_WIDTH-1:0] 	    medeleg_o  		,     
      output logic [DATA_WIDTH-1:0] 	    mideleg_o  		,     
      output logic [DATA_WIDTH-1:0] 	    mie_o 	   		,     
      output logic [DATA_WIDTH-1:0] 	    mtvec_o	   		,	
      output logic [DATA_WIDTH-1:0] 	    mcounter_o 		,     
      output logic [DATA_WIDTH-1:0] 	    mscratch_o 		,	
      output logic [DATA_WIDTH-1:0] 	    mepc_o 	   		,
      output logic [DATA_WIDTH-1:0] 	    mcause_o   		,	
      output logic [DATA_WIDTH-1:0] 	    mtval_o    		,	
      output logic [DATA_WIDTH-1:0] 	    mip_o 			,
      output logic [DATA_WIDTH-1:0]         mcounter_inhibit_o 	,		
      output logic [DATA_WIDTH-1:0]         mcycle_o			,
      output logic [DATA_WIDTH-1:0]         minstret_o		,
      output logic [DATA_WIDTH-1:0]         zic_base_addr_o		,
      output logic [DATA_WIDTH-1:0]         mhpm_counter3_o 		, 
      output logic [DATA_WIDTH-1:0]         mhpm_counter4_o 		, 
      output logic [DATA_WIDTH-1:0]         mhpm_counter5_o 		, 
      output logic [DATA_WIDTH-1:0]         mhpm_counter6_o 		, 
      output logic [DATA_WIDTH-1:0]         mhpm_counter7_o 		, 
      output logic [DATA_WIDTH-1:0]         mhpm_counter8_o 		, 
      output logic [DATA_WIDTH-1:0]         mhpm_counter9_o 		, 
      output logic [DATA_WIDTH-1:0]         mhpm_counter10_o 		, 
      output logic [DATA_WIDTH-1:0]         mhpm_counter11_o		,
      output logic [DATA_WIDTH-1:0]         mhpm_counter12_o		,
      output logic [DATA_WIDTH-1:0]         mhpm_counter13_o		,
      output logic [DATA_WIDTH-1:0]         mhpm_counter14_o		,
      output logic [DATA_WIDTH-1:0]         mhpm_counter15_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter16_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter17_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter18_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter19_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter20_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter21_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter22_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter23_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter24_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter25_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter26_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter27_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter28_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter29_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter30_o		,
      output logic [DATA_WIDTH-1:0]   mhpm_counter31_o	    ,
      output logic [DATA_WIDTH-1:0] mhpm_event3_o ,     
      output logic [DATA_WIDTH-1:0] mhpm_event4_o ,
      output logic [DATA_WIDTH-1:0] mhpm_event5_o ,
      output logic [DATA_WIDTH-1:0] mhpm_event6_o ,
      output logic [DATA_WIDTH-1:0] mhpm_event7_o ,
      output logic [DATA_WIDTH-1:0] mhpm_event8_o ,
      output logic [DATA_WIDTH-1:0] mhpm_event9_o ,
      output logic [DATA_WIDTH-1:0] mhpm_event10_o,
      output logic [DATA_WIDTH-1:0] mhpm_event11_o,
      output logic [DATA_WIDTH-1:0] mhpm_event12_o,
      output logic [DATA_WIDTH-1:0] mhpm_event13_o,
      output logic [DATA_WIDTH-1:0] mhpm_event14_o,
      output logic [DATA_WIDTH-1:0] mhpm_event15_o,
      output logic [DATA_WIDTH-1:0] mhpm_event16_o,
      output logic [DATA_WIDTH-1:0] mhpm_event17_o,
      output logic [DATA_WIDTH-1:0] mhpm_event18_o,
      output logic [DATA_WIDTH-1:0] mhpm_event19_o,
      output logic [DATA_WIDTH-1:0] mhpm_event20_o,
      output logic [DATA_WIDTH-1:0] mhpm_event21_o,
      output logic [DATA_WIDTH-1:0] mhpm_event22_o,
      output logic [DATA_WIDTH-1:0] mhpm_event23_o,
      output logic [DATA_WIDTH-1:0] mhpm_event24_o,
      output logic [DATA_WIDTH-1:0] mhpm_event25_o,
      output logic [DATA_WIDTH-1:0] mhpm_event26_o,
      output logic [DATA_WIDTH-1:0] mhpm_event27_o,
      output logic [DATA_WIDTH-1:0] mhpm_event28_o,
      output logic [DATA_WIDTH-1:0] mhpm_event29_o,
      output logic [DATA_WIDTH-1:0] mhpm_event30_o,
      output logic [DATA_WIDTH-1:0] mhpm_event31_o,
      ...
      output logic [DATA_WIDTH-1:0]   dcsr_o                  ,
      output logic [DATA_WIDTH-1:0]   dpc_o                   ,
    
      input logic                         dbg_csr_write_en_i      ,
      input logic [CSR_ADDR_WIDTH-1:0]    dbg_csr_addr_i          ,
      input logic [DATA_WIDTH-1:0]        dbg_csr_write_data_i    ,
      ...
      output logic [DATA_WIDTH-1:0]  uart_reset_csr_o
       ...
       logic int_exp_valid_w;

       assign int_exp_valid_w = interrupt_valid | exception_valid ;

       logic [7:0]mintststus_w;
       logic [7:0] prv_int_lvl_pr_w ;
    ...
    */
    ...
    begin
    	if(!csr_rst )
    	begin
    		data_mem_max_addr_o <= {{DATA_WIDTH-20{1'b0}},20'h87fff};//87fff{{DATA_WIDTH-20{1'b0}},20'h3ffff}
    	end
        else if(dbg_ndm_reset_i | dbg_hart_reset_i | wdt_reset_i)
        begin
            		data_mem_max_addr_o <= {{DATA_WIDTH-20{1'b0}},20'h87fff};
        end
    	else
    	begin
    ...
    (
      ...
      input logic 		mret_valid_i				,
      input logic [7:0] prv_int_lvl_pr_i ,
      input logic [7:0] 	interrupt_active_level_priority_i	,
      output logic[DATA_WIDTH-1:0] 	interrupt_active_level_priority_o	,
      output logic  [7:0] mintstatus_o       ,
      input logic           dbg_csr_write_en_i      ,
      input logic [CSR_ADDR_WIDTH-1:0]    dbg_csr_addr_i          ,
      input logic [DATA_WIDTH-1:0]    dbg_csr_write_data_i    ,
      input logic           debug_mode_valid_i      ,
    ...
    logic [7:0] fifo [6:0];
    logic [2:0] ptr;
    logic [2:0] trap_entry_cnt;
    logic [2:0] trap_exit_cnt;
    ...
    (
      ...
      input logic 			stall_valid_i				,
      input logic 			branch_valid_i				,
      input logic 		[DATA_WIDTH-1:0] 	minstret_csr_inhibit_bit	,
      output logic [DATA_WIDTH-1:0] 	minstret_o		,
      input logic           dbg_csr_write_en_i      ,
      input logic [CSR_ADDR_WIDTH-1:0]    dbg_csr_addr_i          ,
      input logic [DATA_WIDTH-1:0]    dbg_csr_write_data_i    ,
    ...
    localparam RST_VAL = 0;
    	logic [DATA_WIDTH-1:0] minstret_r;
    ...
    (
      ...
      input logic 	   [CSR_ADDR_WIDTH-1:0] 	csr_write_addr		,
      input logic      [DATA_WIDTH-1:0] 	csr_write_data		,
      input logic 	   [DATA_WIDTH-1:0]		mcycle_csr_inhibit_bit	,
      output logic  [DATA_WIDTH-1:0] 	mcycle_o		,
      input logic           dbg_csr_write_en_i      ,
      input logic [CSR_ADDR_WIDTH-1:0]    dbg_csr_addr_i          ,
      input logic [DATA_WIDTH-1:0]    dbg_csr_write_data_i    ,
    ...
    begin
    	...
    	begin
    		...
    		begin
    			unique case({csr_clear_bit,csr_set_bit})
    ...
    (
      ...
      input logic 			csr_write_en		,
      input logic [CSR_ADDR_WIDTH-1:0] 		csr_write_addr		,
      input logic 			int_exp_valid		,
      input logic 			instr_misalign_valid 	,
      input logic [PC_WIDTH-1:0] 		pc			,
      input logic [PC_WIDTH-1:0]		csr_mepc_in		,
      input logic 			csr_mepc_write_valid	,
      output logic  [PC_WIDTH-1:0]		csr_mepc_o		,	
      output logic  [DATA_WIDTH-1:0] 	mepc_csr_o   	,
      input logic           dbg_csr_write_en_i      ,
      input logic [CSR_ADDR_WIDTH-1:0]    dbg_csr_addr_i          ,
      input logic [DATA_WIDTH-1:0]    dbg_csr_write_data_i    ,
      input logic           debug_mode_valid_i      ,
    ...
    (
      ...
      input logic 			instr_misalign_valid	,
      input logic [DATA_WIDTH-1:0] 		instr_misalign_addr	,
      input logic 			illegal_instr_valid	,
      input logic [INSTRUCTION_WIDTH-1:0] 		illegal_instruction 	,	
      input logic			trap_valid		,//exceptions
      output logic  [DATA_WIDTH-1:0] 	mtval_o   ,
      input logic           dbg_csr_write_en_i      ,
      input logic [CSR_ADDR_WIDTH-1:0]    dbg_csr_addr_i          ,
      input logic [DATA_WIDTH-1:0]    dbg_csr_write_data_i    ,
    ...
    (
      ...
      input logic interrupt_id_write_valid_i,
      output logic [DATA_WIDTH-1:0] mcause_o  ,
      input logic [7:0] mintstatus_i,
      output logic [7:0]prv_int_lvl_pr_o,
      input logic           dbg_csr_write_en_i      ,
      input logic [CSR_ADDR_WIDTH-1:0]    dbg_csr_addr_i          ,
      input logic [DATA_WIDTH-1:0]    dbg_csr_write_data_i    ,
      input logic           debug_mode_valid_i      ,
    ...
    logic [7:0] prev_int_lvl_pr_r;
    logic [DATA_WIDTH-1:0] mcause_r;
    logic [7:0] mintstatus_r;
    ...
    (
      ...
      input logic csr_rst,
      input logic wdt_reset_i,
      output logic  [DATA_WIDTH-1:0] mhart_id_o 
    );
    ...
    (
      ...
      input logic csr_rst,
      input logic wdt_reset_i,
      output  logic [DATA_WIDTH-1:0] mimp_id_o 
    );
    ...
    (
      ...
      input logic csr_rst,
      input logic wdt_reset_i,
      output logic  [DATA_WIDTH-1:0] mvendor_id_o 
    );
    ...
    (
      ...
      input logic csr_rst,
      input logic wdt_reset_i,
      output logic [DATA_WIDTH-1:0] march_id_o 
    );
    ...
    (
      ...
      input logic			csr_rst		,
      input logic wdt_reset_i,
    //  input logic 			csr_write_data	,
    //  input logic 			csr_write_enable,
      output logic  [DATA_WIDTH-1:0] 	misa_csr_o
    );
    ...
    (
      input logic           dcsr_clk_i              ,
      input logic           dcsr_rst_i              ,
      input logic wdt_reset_i,
      input logic [DATA_WIDTH-1:0]    dcsr_write_data_i       ,
      input logic [CSR_ADDR_WIDTH-1:0]    dcsr_addr_i             ,
      input logic           dcsr_write_en_i         ,
      input logic           dcsr_set_en_i           ,
      input logic           dcsr_clear_en_i         ,
    ...
    logic [DATA_WIDTH-1:0] dcsr_r;
    
    always_ff@(posedge dcsr_clk_i or negedge dcsr_rst_i )
    begin
        if(!dcsr_rst_i )   
        begin
    ...
    (
      input logic           dpc_clk_i             ,
      input logic           dpc_rst_i             ,
      input logic wdt_reset_i,
      input logic           dpc_write_en_i        ,
      input logic           dpc_set_en_i          ,
      input logic           dpc_clear_en_i        ,
      input logic [CSR_ADDR_WIDTH-1:0]    dpc_addr_i            ,
      input logic [DATA_WIDTH-1:0]    dpc_write_data_i      ,
      input logic           ebreak_valid_i        ,
      input logic           trigger_valid_i       ,
      input logic           single_step_valid_i   ,
      input logic           haltreq_valid_i       ,
    ...
    logic [DATA_WIDTH-1:0] dpc_r;
       logic [PC_WIDTH-1:0] pc_r;
       logic [PC_WIDTH-1:0] ebreak_pc;
       logic [PC_WIDTH-1:0] stall_pc;
    ...
    //assign pc_r = pc_i;
    /////////////
    logic dbg_hart_reset_r;
       logic neg_dbg_hart_rst;
       logic pos_dbg_hart_req;
    logic haltreq_valid_r;
       logic neg_dbg_hart_req;
    logic branch_valid_r;
    logic stall_valid_r;
    logic [PC_WIDTH-1:0] branch_pc_r;
    always_ff@(posedge dpc_clk_i or negedge dpc_rst_i)
       begin
           if(!dpc_rst_i)
             begin
    ...
    localparam UARTRESET = 12'b0111_1100_0000;//7c0
       logic wr_addr_valid;
    
    
    assign wr_addr_valid = (csr_write_addr == UARTRESET) ? 1'b1 : 1'b0;
    
    always_ff @(posedge csr_clk or negedge csr_rst )
    begin
    	if(!csr_rst )	
        begin
    		uart_reset_csr_o <= {DATA_WIDTH{1'b0}};
    	end
        else if(dbg_ndm_reset_i | dbg_hart_reset_i)
        begin
            		uart_reset_csr_o <= {DATA_WIDTH{1'b0}};
    	...
    	begin
            if(dbg_csr_write_en_i && debug_mode_valid_i && (dbg_csr_addr_i == UARTRESET) )
            begin
                    uart_reset_csr_o <= dbg_csr_write_data_i;
            end
    		else if(csr_write_enable && wr_addr_valid)
    		begin
    			unique case({csr_clear_bit,csr_set_bit})
    				2'b00://complete write
    				begin
    					uart_reset_csr_o <= csr_write_data;
    				end
    				2'b01://clear specified bits
    				begin
    					uart_reset_csr_o <= (uart_reset_csr_o | csr_write_data) ;
    				end
    				2'b10://set specified bits
    				begin
    					uart_reset_csr_o <= (uart_reset_csr_o & (~csr_write_data));
