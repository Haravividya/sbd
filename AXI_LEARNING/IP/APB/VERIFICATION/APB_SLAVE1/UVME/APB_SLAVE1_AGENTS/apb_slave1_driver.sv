...
class apb_slave1_driver extends uvm_driver #(apb_slave1_seq_item);
  
  //virtual interface handle : required to connect interface of DUT - get from config db
  virtual apb_slave1_interface intf; //apb_slave1_intf - set label

  static apb_slave1_seq_item seq_item;
  
  
  `uvm_component_utils(apb_slave1_driver) 

  function new (string name ="apb_slave1_driver",uvm_component parent);
