...
//pslverr     	     
               ...
               if(data.presetn==1 && data.paddr>='h100)
                   begin
                    k=pslverr1_sb;
                    pslverr1_sb=1;
                   end
               ...
               if(data.presetn==1)
                	if (data.psel1 ==0 || data.penable==0)
                       begin
                          pslverr1_sb=k;
                          k=pslverr1_sb;
                       end
               ...
               if(data.presetn==1 && data.psel1 ==1 && data.penable && k==1 && pslverr1_sb==0)  
                    begin
                        if(data.pwrite==1)
                          begin
						  k=pslverr1_sb;
...
//`uvm_info( "k",$sformatf("k:%d" , k), UVM_LOW)
