...
##Command to run this scrpit with coverage : "perl ref_regression.pl <test_list_m.f> <1>"
# test_list_m.f : Create a file named test_list_m.f and inside that write all test names and number for how many times those tests should run.
#                eg: apb_test 100
#
# 1 : This argument is given if you want coverage report of that test run 

##Command to run this scrpit without coverage : "perl ref_regression.pl <test_list_m.f>"
# test_list_m.f : Create a file named test_list_m.f and inside that write all test names and number for how many times those tests should run.
#                eg: apb_test 100
...
sub simulate_test 
{
  ...
  my ($test) = @_;
   {   
    ...
    {
      system("xrun -access +rwc -f ../UVME/APB_MASTERS/comp_file.f -svseed $seed +UVM_TESTNAME=$test  +define+UVM_REPORT_DISABLE_FILE_LINE -coverage all -covdut APB_Master -covworkdir /cov_work -covoverwrite -covfile ./cov_files/cov_cmd.cf -uvmhome CDNS-1.1d"); 
    }
    else
    {  
      system("xrun -access +rwc -f ../UVME/APB_MASTERS/comp_file.f -svseed $seed +UVM_TESTNAME=$test +define+UVM_REPORT_DISABLE_FILE_LINE -uvmhome CDNS-1.1d"); 
    }
