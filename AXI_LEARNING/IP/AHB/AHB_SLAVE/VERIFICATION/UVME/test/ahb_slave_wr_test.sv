...
Description: This is the module which has the test cases for write operation
File name:ahb_slave_wr_test.sv
************************************************************************/

//user defined class derived from base class 
class ahb_slave_wr_test extends ahb_slave_test_tb;

  //factory registration
  `uvm_component_utils(ahb_slave_wr_test) 

  //constructor
  function new(string name="ahb_slave_wr_test",uvm_component parent=null);
  ...
  task run_phase(uvm_phase phase);
    ...
    `uvm_info(get_name(),$sformatf("step 1 : inside test = ahb_slave_wr_test"),UVM_MEDIUM);
    ...
    begin      
      ...
      //scenario 2-----------------------UNDEFIND LENGTH BEAT (16)
	  seq_inst.scenario = 2;
	  seq_inst.hreset_n_seq = 1;
      ...
      //scenario 2-----------------4 BEAT INC
	  seq_inst.scenario = 2;                                                                                                                        
	  seq_inst.hreset_n_seq = 1;
      seq_inst.haddr_seq =$urandom_range(0,28);
      ...
      //scenario 2-----------------8 BEAT INC
	  seq_inst.scenario = 2;                                                                                                                        
	  seq_inst.hreset_n_seq = 1;
      seq_inst.haddr_seq =$urandom_range(0,24);
    ...
    `uvm_info(get_name(),$sformatf("step 6 : inside test done = ahb_slave_wr_test"),UVM_MEDIUM);
