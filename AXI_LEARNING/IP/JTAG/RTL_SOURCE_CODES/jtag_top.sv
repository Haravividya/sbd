...
module jtag_top 
  #(parameter           DWIDTH  = 32   , // DATA WIDTH for DMI Register 
	...
	output logic              dr_scan        , // DTMCS Register 16th pin
	...
	//  logic declarations for interconnnected blocks
	 ...
	 logic               Capture_DR    ;
	 logic               Shift_DR_Out_In      ;
	 logic               Update_DR_Out_In     ;
	
	 logic               Capture_IR_Out_In    ;
	 logic               Shift_IR_Out_In      ;
	 logic               Update_IR_Out_In     ;
	
	 logic [IR_BITS-1:0] instr_reg_out_in     ;
	 logic               ir_shift_out_in      ;
	
	 logic               bypass_sel           ;
	 logic               bypass_shift_out_in  ;
	
	 logic               idcode_sel           ;
	 logic               idcode_shift_out_in  ;
	
	 logic               dtm_csr_sel          ;
	 logic               dtm_csr_shift_out_in ;
	
	 logic               dmi_access_sel       ;
	 logic               dmi_shift_out_in     ;
	
//	 logic               command_sel          ;
//	 logic               command_shift_out_in ;
     ...
     */
	 ...
	 // IR output logic signals
	 .instr_reg_out (instr_reg_out_in ), // Instruction Register updates its value at Update Stage
	 .ir_shift_out  (ir_shift_out_in  ) // IR Shift Output signal latched on negedge TCK after output logic MUX
...
//	 .command_shift_in   (command_shift_out_in ), // COMMAND bits shifts to TDO
	 ...
	 //.tdo_en             (tdo_en               )  // TDO output logic enable enables TDO output logic data
	                                           );
	/* 
	  =====================	End of TDO_Mux =====================
	                                                            */
