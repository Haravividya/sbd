...
sub simulate_test 
{
  ...
  my ($test) = @_;
   {   
    ...
    {
      system("xrun -access +rwc -f ../UVME/debug_compile.f -svseed $seed +UVM_TESTNAME=$test  +define+UVM_REPORT_DISABLE_FILE_LINE -coverage all -covdut dtm_to_dm -covworkdir /cov_work -covoverwrite -covfile ./cov_files/cov_cmd.cf -uvmhome CDNS-1.1d"); 
    }
    else
    {  
      system("xrun -access +rwc -f ../UVME/debug_compile.f -svseed $seed +UVM_TESTNAME=$test +define+UVM_REPORT_DISABLE_FILE_LINE -uvmhome CDNS-1.1d"); 
    }
